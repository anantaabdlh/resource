...
mkdir -p $HOME/$LUMEN_FOLDER/$BINARY/genesis/bin
sudo mv $HOME/go/bin/$LUMEN $HOME/$LUMEN_FOLDER/$BINARY/genesis/bin/

# Create application symlinks
ln -s $HOME/$LUMEN_FOLDER/$BINARY/genesis $HOME/$LUMEN_FOLDER/$BINARY/current
sudo ln -s $HOME/$LUMEN_FOLDER/$BINARY/current/bin/$LUMEN /usr/bin/$LUMEN
...
RestartSec=10
LimitNOFILE=65535
Environment="DAEMON_HOME=$HOME/$LUMEN_FOLDER"
Environment="DAEMON_NAME=$LUMEN"
...
echo ""
echo -e "CHECK STATUS BINARY : \e[1m\e[31msystemctl status $LUMEN\e[0m"
