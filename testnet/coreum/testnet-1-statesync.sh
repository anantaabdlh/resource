...
echo "░  ░  ░   ░    ░    ░ ░   ░      ░      ░   ░ ░   ░   ░ ░        ";
echo "      ░   ░    ░      ░  ░       ░            ░     ░          ░ ";
echo "   Auto Installer coreum-testnet-1 For COREUM FOUNDATION v0.1.1  ";
echo -e "\e[0m"
sleep 1
...
[Service]
User=$USER
ExecStart=$(which $TESTCORE) start
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
...
touch $HOME/$TESTCORE_FOLDER/$TESTCORE_ID/config/genesis.json
...
# Set config snapshot
sed -i -e "s/^snapshot-interval *=.*/snapshot-interval = \"1000\"/" $HOME/$TESTCORE_FOLDER/$TESTCORE_ID/config/app.toml
sed -i -e "s/^snapshot-keep-recent *=.*/snapshot-keep-recent = \"2\"/" $HOME/$TESTCORE_FOLDER/$TESTCORE_ID/config/app.toml

# Enable state sync
$TESTCORE tendermint unsafe-reset-all
...
s|^(trust_hash[[:space:]]+=[[:space:]]+).*$|\1\"$TRUST_HASH\"|" $HOME/$TESTCORE_FOLDER/$TESTCORE_ID/config/config.toml
