...
echo "export CORRINO_GENESIS=${CORRINO_GENESIS}" >> $HOME/.bash_profile
echo "export CORRINO_ADDRBOOK=${CORRINO_ADDRBOOK}" >> $HOME/.bash_profile
...
curl -Ls $CORRINO_GENESIS > $HOME/$CORRINO_FOLDER/config/genesis.json
curl -Ls $CORRINO_ADDRBOOK > $HOME/$CORRINO_FOLDER/config/addrbook.json
